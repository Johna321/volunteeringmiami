        ...
        return(
            <div className="stuff">
                ...
                <button className="btn btn-dark" onClick={()=>{
                    fetch(`http://209.97.154.166/sendgrid?Body=${this.state.Body}&Email=${this.props.email}&Subject=${this.state.Subject}&Name=${this.state.Name}&SenderEmail=${this.state.SenderEmail}`)
                    .then(res => res.json())
                    .then((result) => {this.successfulSubmit()},(error) => {})
                }}>Submit</button>
            </div>
        );
    }
